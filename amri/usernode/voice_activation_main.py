...
def main():
    ...
    #Read CSV from Dataframe
    dfquestions = pandas.read_excel(r'..\Questions.xlsx', dtype={'Questions': str})
    dfquestions['Questions'].astype('str')

    for index, question in dfquestions.iterrows():
        log(question['Questions'], question['Reply'])
        google_tts_obj.speak(question['Questions'])
        if question['Reply'] == 'Yes':
            answer = google_stt_obj.listen()


    #Introduction
    text = 'Welcome to MRI Safety Zone II. Here we will ask some questions including your medical history, explain the MRI procedure including the risk and hazards associated with MRI, and then hopefully proceed to the test. Would you like to continue?'

    google_tts_obj.speak(text)
    introContinue = google_stt_obj.listen()

    if(introContinue) == 'yes':
        ...
        return "Safety Zone II Assessment not completed successfully"



def yesnoquesion(question, google_tts_obj, google_stt_obj):
    log(question)
    google_tts_obj.speak(question)
    return google_stt_obj.listen()
